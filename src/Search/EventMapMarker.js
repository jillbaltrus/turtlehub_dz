...
import {
  ...
  AlertDialogOverlay, Center, useDisclosure
...
function EventMapMarker({ event }) {
  ...
  useEffect(() => {
    getCoordinates(event.location).then(coordinates => setLocation(coordinates));
  }, []);
  ...
  return (
      ...
      <MarkerF key={event._id}
          position={location} name={event.title} onClick={handleMarkerClick}>
      </MarkerF>
      </>
  );
}
